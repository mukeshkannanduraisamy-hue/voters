...
export default function Survey() {
  ...
  const seedAnswers = (v: Voter, s: FormSchema | null): AnswerMap => {
    ...
    // defaulting to "Others" when no prior survey exists.
    const sectorField = s.fields.find((f) => f.key === 'job_sector' || (f.source?.kind === 'master' && f.source.master === 'job_sector'));
    if (sectorField) out[sectorField.key] = survey?.jobCategory || 'Others';
