 ...
 * survey form. Available to every role.
 */
...
router.get('/all', requireRole(ROLES.A1), async (req, res, next) => {
  try {
    const rows = await db
      .prepare(
...
router.post('/', requireRole(ROLES.A1), async (req, res, next) => {
  try {
    ...
    const exists = await db.prepare('SELECT 1 FROM survey_field_defs WHERE field_key = ?').get(key);
    if (exists) {
      key = `${key}_${Date.now().toString(36)}`;
    }

    const maxRow = await db.prepare('SELECT COALESCE(MAX(sort_order), 0) m FROM survey_field_defs').get();
    ...
    const row = await db.prepare('SELECT * FROM survey_field_defs WHERE id = ?').get(info.lastInsertRowid);
...
router.patch('/:id', requireRole(ROLES.A1), async (req, res, next) => {
  try {
    ...
    const existing = await db.prepare('SELECT * FROM survey_field_defs WHERE id = ?').get(id);
    ...
    await db.prepare(`UPDATE survey_field_defs SET ${sets.join(', ')} WHERE id = ?`).run(...params, id);
    audit(req.user.id, 'FORM_FIELD_UPDATED', 'survey_field_defs', id, JSON.stringify(Object.keys(req.body ?? {})));
    const row = await db.prepare('SELECT * FROM survey_field_defs WHERE id = ?').get(id);
...
/** POST /api/form-fields/:id/move — swap sort_order with the adjacent field (A1 only). */
router.post('/:id/move', requireRole(ROLES.A1), async (req, res, next) => {
  try {
    const id = Number(req.params.id);
    const direction = String(req.body?.direction ?? '').trim();
    if (!['up', 'down'].includes(direction)) return res.status(400).json({ error: 'direction must be "up" or "down"' });

    const all = await db.prepare('SELECT id, sort_order FROM survey_field_defs ORDER BY sort_order, id').all();
    ...
    await db.prepare('UPDATE survey_field_defs SET sort_order = ? WHERE id = ?').run(b.sort_order, a.id);
    await db.prepare('UPDATE survey_field_defs SET sort_order = ? WHERE id = ?').run(a.sort_order, b.id);
    res.json({ ok: true });
  } catch (err) {
    next(err);
  }
...
router.delete('/:id', requireRole(ROLES.A1), async (req, res, next) => {
  try {
    const id = Number(req.params.id);
    const usedRow = await db.prepare('SELECT COUNT(*) c FROM survey_field_values WHERE field_id = ?').get(id);
    ...
    const info = await db.prepare('DELETE FROM survey_field_defs WHERE id = ?').run(id);
