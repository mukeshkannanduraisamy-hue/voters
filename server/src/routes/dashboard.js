...
router.get('/stats', async (req, res, next) => {
  try {
    const sKey = await scopeKey(req.user);
    const key = `stats|${sKey}`;
    const payload = await cached(key, async () => {
      ...
      const onDay = async (daysAgo) => {
        const dateCond = daysAgo === 0
          ? "DATE(s.updated_at) = CURDATE()"
          : `DATE(s.updated_at) = DATE_SUB(CURDATE(), INTERVAL ${daysAgo} DAY)`;
      ...
      const today = await onDay(0);
      const yesterday = await onDay(1);

      const localBodiesRows = await db
        .prepare(
      ...
      const localBodies = localBodiesRows.map((r) => {
        const tot = Number(r.total || 0);
        const comp = Number(r.completed || 0);
        return {
          name: r.name,
          type: r.type,
          booths: Number(r.booths || 0),
          total: tot,
          completed: comp,
          pending: tot - comp,
          progress: pct(comp, tot),
        };
      });
      ...
      const trendRows = await db
        .prepare(
          `SELECT DATE(s.updated_at) AS day, COUNT(*) AS count
             ...
             JOIN voters_master v ON v.epic_id = s.epic_id
            WHERE ${scope.sql} AND DATE(s.updated_at) >= DATE_SUB(CURDATE(), INTERVAL 13 DAY)
            GROUP BY day ORDER BY day`
        )
        .all(...scope.params);

      const trendMap = new Map(trendRows.map((t) => [String(t.day).slice(0, 10), Number(t.count || 0)]));
      const trend = [];
      for (let i = 13; i >= 0; i--) {
        const d = new Date();
      ...
      const boothCountRow = await db.prepare(`SELECT COUNT(DISTINCT v.part_no) c FROM voters_master v WHERE ${scope.sql}`).get(...scope.params);
      ...
      return {
        ...
        totals: {
          ...
          booths: Number(boothCountRow?.c || 0),
...
router.get('/agents', requireRole(ROLES.A1, ROLES.A2), async (req, res, next) => {
  try {
    ...
    res.json(
      rows.map((r) => {
        const done = Number(r.surveysDone || 0);
        const assigned = Number(r.assignedVoters || 0);
        return {
          id: r.id,
          ...
          surveysDone: done,
          todayDone: Number(r.todayDone || 0),
          assignedVoters: assigned,
          boothCount: Number(r.boothCount || 0),
          partList: r.partList ? String(r.partList).split(',').map(Number) : [],
...
router.get('/breakdown', async (req, res, next) => {
  try {
    ...
    const payload = await cached(key, async () => {
      ...
      return {
        castes: castes.map(r => ({ ...r, count: Number(r.count || 0) })),
        jobSectors: jobSectors.map(r => ({ ...r, count: Number(r.count || 0) })),
        jobs: jobs.map(r => ({ ...r, count: Number(r.count || 0) })),
        parties: parties.map(r => ({ ...r, count: Number(r.count || 0) })),
        gender: gender.map(r => ({ ...r, count: Number(r.count || 0) })),
        ageBands: ageBands.map(r => ({ ...r, count: Number(r.count || 0) })),
      };
    });
...
router.get('/audit', requireRole(ROLES.A1), async (req, res, next) => {
  try {
